...
+ (UIImage *)imageWithImage:(UIImage *)image cropMiddleSegmentPercentage:(float)perc {
    CGRect imageRect = (CGRect){CGPointZero, image.size};
    float middle = imageRect.size.height / 2;
    float delta = middle * (perc/100);
    CGRect cropRect = CGRectMake(0.0f, middle - delta, imageRect.size.width, delta*2);
    return [UIImage imageWithImage:image cropInRect:CGRectIntegral(cropRect)];
}
